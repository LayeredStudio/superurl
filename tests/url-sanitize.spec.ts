...
describe('sanitizeUrl()', function () {
	...
	it('allow ftp protocol', function () {
		assert.equal(sanitizeUrl('ftp://ftp.host', { allowedProtocols: ['http:', 'https:', 'ftp:'] }), 'ftp://ftp.host/');
	});
	...
	it('no protocol, append http://', function () {
		assert.equal(sanitizeUrl('google.com'), 'https://google.com/');
		assert.equal(sanitizeUrl('www.example.com'), 'https://www.example.com/');
	});

	it('upgrade protocol to https on known secure domains', function () {
		assert.equal(sanitizeUrl('http://facebook.com'), 'https://www.facebook.com/');
		assert.equal(sanitizeUrl('http://google.com'), 'https://google.com/');
	});

	it('remove analytics/campaigns tracking params', function () {
		assert.equal(sanitizeUrl('https://www.facebook.com/oculus/?ref=bookmarks'), 'https://www.facebook.com/oculus/');
	...
	it('remove language params', function () {
		assert.equal(sanitizeUrl('https://www.instagram.com/twitter/?hl=en'), 'https://www.instagram.com/twitter/');
	});

	it('remove Meta (Instagram/Facebook/WhatsApp) tracking params', function () {
		assert.equal(sanitizeUrl('https://instagram.com/instagram?igshid=YmMyMTA2M2Y='), 'https://www.instagram.com/instagram');
		assert.equal(sanitizeUrl('https://trailrunningacademy.com/?fbclid=PAAabHWuo5mUnQXPoQ2CdbUdN_S8nJGA11yv8sdxRb5HfSphG_QYAv1f76MSQ'), 'https://trailrunningacademy.com/');
	});

	it('remove Twitter tracking params', function () {
		assert.equal(sanitizeUrl('https://twitter.com/Twitter/status/1601692766257709056?s=20&t=lBdttyHtwzrA27bJqu0g0Q'), 'https://twitter.com/Twitter/status/1601692766257709056');
		assert.equal(sanitizeUrl('https://twitter.com/Twitter/status/1601692766257709056?ref_src=twsrc%5Etfw%7Ctwcamp%5Etweetembed%7Ctwterm%5E1598901113910333440%7Ctwgr%5E61781d6803d9d3ca6533629ca455faeab6f45cd2%7Ctwcon%5Es1_&ref_url=https%3A%2F%2Fdomaininvesting.com%2Fpage%2F2%2F'), 'https://twitter.com/Twitter/status/1601692766257709056');
		assert.equal(sanitizeUrl('https://twitter.com/twitter?s=11&t=2tlSCDpZdGnnGGLxfKXDZA'), 'https://twitter.com/twitter');
	});
